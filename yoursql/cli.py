...
from .engine.ssh import serve_ssh_stdio
...
def _format_result(result: ExecutionResult) -> str:
    ...
    rows = [["NULL" if value is None else str(value) for value in row] for row in data["rows"]]
    widths = [len(column) for column in columns]
    for row in rows:
        widths = [max(width, len(value)) for width, value in zip(widths, row, strict=True)]
    separator = "+" + "+".join("-" * (width + 2) for width in widths) + "+"
    lines = [separator, "|" + "|".join(f" {column:<{width}} " for column, width in zip(columns, widths, strict=True)) + "|", separator]
    lines.extend("|" + "|".join(f" {value:<{width}} " for value, width in zip(row, widths, strict=True)) + "|" for row in rows)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    database_default = os.getenv("YOURSQL_CLI_DATABASE") or os.getenv("YOURSQL_DATABASE") or str(DEFAULT_DATABASE_PATH)
    ...
    parser.add_argument("--password", default=os.getenv("YOURSQL_CLI_PASSWORD", "admin"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        with Database(args.database, config=database_config,
                      user=args.user, password=args.password) as database:
            if args.stdio:
            ...
            if sql is not None:
                ...
                for result in results:
                    print(json.dumps(result.as_dict(), ensure_ascii=False) if args.json else _format_result(result))
                return 0
            while True:
                try:
                    line = input("yoursql> ")
