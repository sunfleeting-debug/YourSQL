...
from ..common import AuthorizationError, DatabaseConfig, YourSQLError, RuntimeConfig
...
from .workbench_sql import (SQLSlice, compile_observed, error_info,
                            redact_sql, result_columns, split_sql, stage)
...
class Workbench:
    ...
    def __init__(self, database: Database, *, session_ttl: float | None = None,
        ...
        self._lock = RLock()
        self._worker = ThreadPoolExecutor(max_workers=1, thread_name_prefix="yoursql-query")
    ...
    def login(self, username: str, password: str) -> tuple[str, WebSession]:
        with self.connection(None):
            ...
            with self._lock:
                ...
                session = WebSession(self._key(token), Session(user, self.database.rbac),
            ...
            for key, session in tuple(self._sessions.items()):
                if session.closed or monotonic() >= session.expires_at:
                    ...
                    for task_id, task in tuple(self._tasks.items()):
                        if task.session_key == key and task.state not in {"queued", "running"}:
    ...
    def me(self, session: WebSession) -> dict[str, object]:
        return {"user": session.connection.user.name, "roles": sorted(session.connection.user.roles),
                "permissions": list(self.database.rbac.privileges_for(user=session.connection.user.name)),
                "database": self.database.path.name, "database_path": self._display_path(self.database.path),
                "session_expires_in": max(0, int(session.expires_at - monotonic())),
                "active_task": session.active_task}

    def metadata(self, session: WebSession, name: str | None = None) -> dict[str, object]:
        with self.connection(session):
            ...
            for table in selected:
                try:
                    ...
                    continue
                item = table.to_dict()
                item["indexes"] = [index.to_dict() for index in self.database.catalog.indexes() if index.table_id == table.table_id]
                item["create_sql"] = self.database._show(Show("CREATE_TABLE", table.name)).rows[0][1]
                tables.append(item)
            if name is None:
                for view in self.database.catalog.views():
                    try:
                        session.connection.authorize("SELECT", view.name)
                        self.database._authorize_statement(self.database._view_query(view), "SELECT")
                    except AuthorizationError:
                        continue
                    item = view.to_dict()
                    item["create_sql"] = self.database._show(Show("CREATE_VIEW", view.name)).rows[0][1]
                    views.append(item)
            return {"databases": [{"name": self.database.path.name, "tables": tables, "views": views}],
                    "single_database": True, "filtered_by": "SELECT", "refreshed_at": now()}
    ...
    def _database_files(self) -> dict[str, object]:
        ...
        try:
            candidates = sorted(
                (path for path in root.iterdir() if path.is_file() and path.suffix.lower() == ".db"),
        ...
        for path in candidates:
            try:
                ...
                continue
            files.append({"name": path.name, "path": self._display_path(path), "size_bytes": size_bytes,
                          "active": resolved == active})
        return {"active": self.database.path.name, "active_path": self._display_path(self.database.path),
                "files": files, "limit": 100,
                "note": "列表来自当前数据库所在目录；也可以输入本机上的 .db 路径。"}

    def database_files(self, session: WebSession) -> dict[str, object]:
        """列出已登录用户可切换的数据库文件。"""
    ...
    def public_database_files(self) -> dict[str, object]:
    ...
    def _database_candidate(self, value: str, *, allow_external: bool = True,
    ...
    @staticmethod
    def _config_integer(options: dict[str, object], name: str, default: int,
    ...
    def _new_database_config(self, options: dict[str, object]) -> DatabaseConfig:
        """把工作台创建表单转换成内核配置。"""

        defaults = self.settings.database_config
        page_size = self._config_integer(options, "page_size", defaults.page_size, 512, 65_536)
        if page_size & (page_size - 1):
            raise YourSQLError("page_size 必须是 512–65536 之间的二次幂", "BAD_REQUEST")
        buffer_pool_size = self._config_integer(options, "buffer_pool_size", defaults.buffer_pool_size, 1, 4096)
        ...
        return DatabaseConfig(page_size=page_size, buffer_pool_size=buffer_pool_size,
                              replacement_policy=policy.lower(), max_varchar_length=defaults.max_varchar_length)

    @staticmethod
    def _config_dict(config: DatabaseConfig) -> dict[str, object]:
        """返回创建结果中可展示的内核配置。"""

        return {"page_size": config.page_size, "buffer_pool_size": config.buffer_pool_size,
        ...
        try:
            ...
            with self._lock:
                if any(task.state in {"queued", "running"} for task in self._tasks.values()):
                    raise YourSQLError("当前仍有 SQL 任务执行，请等待完成后再切换", "SERVICE_BUSY")
    ...
    def _select_database(self, session: WebSession | None, path: str, *, require_security: bool) -> dict[str, object]:
        ...
        with self._switch_lock:
            with self.connection(session):
                if require_security:
                    ...
                    session.connection.authorize("SECURITY")
                candidate = self._database_candidate(path, allow_external=require_security)
                current = self.database
                if candidate == current.path.resolve():
                    return {"database": current.path.name, "path": self._display_path(current.path),
                            "previous_database": current.path.name, "previous_path": self._display_path(current.path),
                            "changed": False, "requires_login": False}
                try:
                    target_page_size = Database.detect_page_size(candidate)
                    target_config = current.config
                    if target_page_size is not None and target_page_size != current.config.page_size:
                        target_config = replace(current.config, page_size=target_page_size)
                    replacement = Database(candidate, config=target_config)
                except Exception as exc:
                    raise YourSQLError("无法打开数据库文件，文件可能损坏或不是 YourSQL 数据库", "BAD_REQUEST") from exc
            previous, selected = self._replace_database(replacement)
            return {"database": selected, "path": self._display_path(self.database.path),
                    "previous_database": previous, "changed": True, "requires_login": True}

    def select_database(self, session: WebSession, path: str) -> dict[str, object]:
        """切换到用户指定的数据库；切换成功后要求在新库重新登录。"""

        return self._select_database(session, path, require_security=True)

    def select_database_before_login(self, path: str) -> dict[str, object]:
        """为登录页切换当前服务目录数据库；切换后必须使用目标库账号登录。"""

        return self._select_database(None, path, require_security=False)

    def create_database(self, session: WebSession, path: str,
        ...
        with self._switch_lock:
            with self.connection(session):
                session.connection.authorize("SECURITY")
                candidate = self._database_candidate(path, allow_external=True, must_exist=False)
                if candidate.exists():
                    raise YourSQLError("目标数据库已存在，请换一个文件名", "BAD_REQUEST")
                ...
                try:
                    ...
                    raise YourSQLError("无法创建数据库，请检查路径和配置", "BAD_REQUEST") from exc
            previous, selected = self._replace_database(replacement)
            return {"database": selected, "path": self._display_path(self.database.path),
                    "previous_database": previous, "changed": True, "requires_login": True,
                    "config": self._config_dict(config)}

    def _import_database(self, session: WebSession | None, filename: str,
                         content: bytes, *, require_security: bool) -> dict[str, object]:
        ...
        with self._switch_lock:
            with self.connection(session):
                ...
                if candidate.exists():
                    raise YourSQLError("当前数据库目录已有同名文件，请改用路径选择", "BAD_REQUEST")
                ...
                try:
                    ...
                    if replacement is not None:
                        replacement.close()
                    raise YourSQLError("无法导入数据库，文件可能损坏或不是 YourSQL 数据库", "BAD_REQUEST") from exc
                finally:
    ...
    def import_database(self, session: WebSession, filename: str, content: bytes) -> dict[str, object]:
        """登录后导入资源管理器选中的数据库。"""
        return self._import_database(session, filename, content, require_security=True)

    def import_database_before_login(self, filename: str, content: bytes) -> dict[str, object]:
        """登录前导入已有数据库，导入后仍需使用目标库账号登录。"""
        return self._import_database(None, filename, content, require_security=False)

    def set_storage_policy(self, session: WebSession, replacement_policy: str) -> dict[str, object]:
        ...
        with self._switch_lock:
            with self.connection(session):
                ...
                if changed:
                    ...
                    self.database.config = replace(self.database.config, replacement_policy=policy)
                buffer_pool = self.database.buffer_pool.snapshot(0, 100)
                return {"snapshot_at": now(), "changed": changed, "previous_policy": previous,
                        "replacement_policy": policy, "buffer_pool": buffer_pool,
                        "note": "策略仅影响当前服务进程；现有缓存帧和累计统计保持不变，下一次淘汰开始采用新策略。"}

    def dialect(self) -> dict[str, object]:
        return {"keywords": sorted(set(KEYWORDS) | {"NULL", "TRUE", "FALSE"}),
                "types": ["INT", "INTEGER", "FLOAT", "DOUBLE", "REAL", "VARCHAR", "TEXT", "STRING", "BOOL", "BOOLEAN"],
                "functions": ["COUNT", "SUM", "AVG", "MIN", "MAX", "ABS", "LOWER", "UPPER", "LENGTH", "COALESCE", "DATE"],
                "limits": {"sql_chars": self.settings.max_sql_chars, "statements": self.settings.max_statements,
                            "result_rows": self.settings.max_result_rows, "page_size": 500,
                            "history_entries": self.settings.history_entries,
                            "timeout_seconds": self.settings.max_query_timeout_seconds},
                "limitations": ["提示基于关键字和可见 Schema，不是完整语义语言服务器。",
                                "AST 暂不提供完整节点区间；有 Token 位置的语义错误精确定位，其余错误回退到语句起点。",
                                "表达式类型根据结果推断，全 NULL 或空表达式列显示 UNKNOWN。"]}

    def validate(self, session: WebSession, sql: str) -> dict[str, object]:
        ...
        for source in statements:
            try:
                ...
                diagnostics.append(error_info(exc, source, sensitive="IDENTIFIED" in source.sql.upper()))
        return {"valid": not diagnostics, "diagnostics": diagnostics,
                "statements": [source.location() for source in statements],
                "scope": "syntax_only", "note": "这里只校验语法；执行时检查权限、表列与类型。"}

    def submit(self, session: WebSession, sql: str, timeout: float, row_limit: int) -> QueryTask:
        ...
        with self._lock:
            ...
            if len(self._tasks) >= self.settings.max_tasks:
                completed = next((key for key, task in self._tasks.items() if task.state not in {"queued", "running"}), None)
                if completed is None:
                    raise YourSQLError("任务队列已满", "RESOURCE_LIMIT")
                self._tasks.pop(completed)
            task = QueryTask(uuid4().hex, session.key, sql, now(), timeout, row_limit, deadline=monotonic() + timeout)
    ...
    def _run(self, session: WebSession, task: QueryTask) -> None:
        ...
        try:
            with self.connection(session):
                ...
                for source in split_sql(task.sql, max_statements=self.settings.max_statements):
                    # WHY：默认不截断流水线步数，避免复杂 JOIN 丢失执行信息；可用 YOURSQL_TRACE_MAX_STEPS 恢复上限。
                    trace = ExecutionTrace(min(task.deadline, session.expires_at), task.cancel,
                                           max_steps=self.settings.trace_max_steps)
                    stages: list[dict[str, object]] = []
                    statement_started = perf_counter()
                    item: dict[str, object] = {"sql": redact_sql(source.sql), "source": source.location(),
                                               "stages": stages, "columns": [], "rows": [], "affected_rows": 0}
                    ...
                    try:
                        ...
                        trace.interruptible = isinstance(statement, (Select, Show, Explain))
                        before_io = self.database.disk.io_stats()
                        before_buffer = self.database.buffer_pool.stats()
                        execution_started = perf_counter()
                        result = self.database._execute_compilation(compilation, optimized_plan=compilation.optimized_plan)
                        ...
                        plan_estimate = None
                        if compilation.optimized_plan is not None and "IDENTIFIED" not in source.sql.upper():
                            estimate = self.database.estimate_plan(compilation.optimized_plan)
                            plan_estimate = {"startup_cost": estimate.startup_cost,
                                             "total_cost": estimate.total_cost, "rows": estimate.rows,
                                             "unit": "cost", "model": "optimizer_statistics"}
                        item.update({"status": "success", "columns": result_columns(self.database, statement, result),
                                     "rows": rows, "total_rows": len(result.rows), "retained_rows": len(rows),
                                     "truncated": len(rows) < len(result.rows), "affected_rows": result.affected_rows,
                                     "message": result.message, "execution_ms": execution_ms,
                                     "plan": compilation.plan.to_dict() if "IDENTIFIED" not in source.sql.upper() else None,
                                     "stats": result.stats, "plan_estimate": plan_estimate})
                        after_io = self.database.disk.io_stats()
                        after_buffer = self.database.buffer_pool.stats()
                        statistics = {"engine": result.stats, "execution_ms": execution_ms,
                                      "io_delta": {key: after_io[key] - before_io[key] for key in after_io},
                                      "buffer_delta": {key: after_buffer[key] - before_buffer[key] for key in ("hits", "misses", "evictions")},
                                      "returned_rows": len(result.rows), "affected_rows": result.affected_rows,
                                      "observed_steps": trace.steps}
                        self._finish_stages(stages, source, trace, statistics)
                    except Exception as exc:
                        error = error_info(exc, source, sensitive="IDENTIFIED" in source.sql.upper())
                        item.update({"status": "error", "error": error, "total_rows": 0, "retained_rows": 0})
                        if not stages or stages[-1]["status"] != "error":
                            stages.append(stage("execution", "error", source, error=error,
                                                duration_ms=(perf_counter() - statement_started) * 1000))
                        task.error = error
                    finally:
                    ...
                    for artifact in stages:
                        if len(json.dumps(artifact, ensure_ascii=False).encode()) > 128_000:
                            artifact.update({"data": {"truncated": True}, "text": "产物超过 128 KiB，已省略；请缩短 SQL。", "truncated": True})
                    with self._lock:
                        task.results.append(item)
                        self._history.setdefault(session.connection.user.name.lower(),
                                                 deque(maxlen=self.settings.history_entries)).appendleft({
                            "id": uuid4().hex, "task_id": task.id, "sql": item["sql"], "executed_at": now(),
                            "status": item["status"], "elapsed_ms": item["elapsed_ms"],
                            "error_summary": str(task.error["code"]) if task.error else None,
                            "redaction": "注释和字面量已移除，历史仅作追溯，不可直接重放。"})
                    if task.error:
                        break
                with self._lock:
                    task.state = ("cancelled" if task.error and task.error["code"] == "CANCELLED" else
                                  "timeout" if task.error and task.error["code"] == "TIMEOUT" else
                                  "error" if task.error else "success")
        except Exception as exc:
            with self._lock:
    ...
    @staticmethod
    def _finish_stages(stages: list[dict[str, object]], source: SQLSlice, trace: ExecutionTrace,
                       statistics: dict[str, object]) -> None:
        stages.extend([
            stage("executor", "partial", source, data={"implementation": "Database SQL evaluator", "scans": trace.scans},
                  reason="SQL evaluator 已消费优化后的扫描路径；Volcano 算子树仍未作为查询主执行器实例化。"),
            stage("storage", "success", source, data={"events": trace.events, "dropped_events": trace.dropped_events},
                  reason="来自 BufferPool/DiskManager 的请求内访问事件，最多 256 项；未单独测量每次存储耗时。"),
            stage("statistics", "success", source, data=statistics, duration_ms=float(statistics["execution_ms"])),
        ])

    def task(self, session: WebSession, task_id: str, *, result_index: int | None = None,
             offset: int = 0, limit: int = 100) -> dict[str, object]:
        with self._lock:
            ...
            return {"id": task.id, "status": task.state, "submitted_at": task.submitted_at,
                    "elapsed_ms": task.elapsed_ms, "error": task.error, "cancel_requested": task.cancel.is_set(),
                    "results": [{key: value for key, value in item.items() if key not in {"rows", "stages", "plan", "stats"}}
                                for item in task.results]}

    def cancel(self, session: WebSession, task_id: str) -> dict[str, object]:
        with self._lock:
            self.task(session, task_id)
            task = self._tasks[task_id]
            if task.state in {"queued", "running"}:
                task.cancel.set()
            return {"id": task.id, "status": task.state, "cancel_requested": task.cancel.is_set(),
                    "note": "只读查询可协作取消；写语句执行中不会强制中断，后续语句将在边界停止。"}

    def history(self, session: WebSession, offset: int, limit: int) -> dict[str, object]:
        with self._lock:
