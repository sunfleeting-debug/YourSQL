...
from ..storage.page import HEADER_SIZE, PAGE_VERSION, SLOT_ENTRY_SIZE, Page, PageType, SlottedPage
...
def _index_pages(database: Database) -> dict[int, list[tuple[str, TableMetadata | None]]]:
    """建立索引物理页到索引目录项的反向映射。"""

    tables = {table.table_id: table for table in database.catalog.tables(include_system=True)}
    bindings: dict[int, list[tuple[str, TableMetadata | None]]] = {}
    for metadata in database.catalog.indexes():
        try:
            page_ids = database.index_manager.get(metadata.name).physical_page_ids(readonly=True)
        except YourSQLError:
...
def page_header(
    ...
    *,
    ...
    result: dict[str, object] = {
        "page_id": page.page_id, "type": page.page_type.value, "page_size": page.page_size,
        "header_size": HEADER_SIZE, "payload_size": len(page.payload), "free_space": page.free_space,
        "magic": "MDBP", "version": PAGE_VERSION, "crc32": f"{zlib.crc32(page.payload) & 0xFFFFFFFF:08x}",
    }
    if page.page_type is PageType.HEAP:
        slotted = SlottedPage.from_page(page)
        result.update({"free_space": slotted.free_space,
        ...
        try:
            ...
            result.update({"index_format": node["format"], "index_node_type": node["node_type"],
                           "index_level": node["level"], "index_key_count": node["key_count"]})
        except YourSQLError:
    ...
    if database is not None:
        ...
        if page.page_type is PageType.INDEX:
            ...
            for index_name, index_table in bindings.get(page.page_id, []):
                is_masked = index_table is not None and index_table.system and not _is_admin(database)
                result["index_name"] = index_name if not is_masked else "MASKED"
                result["table_name"] = (
                    index_table.name if index_table is not None and not is_masked else "MASKED"
                )
                result["system_table"] = index_table.system if index_table is not None else False
                result["masked"] = is_masked
                break
    return result


def storage_snapshot(database: Database, offset: int, limit: int, *, map_only: bool = False) -> dict[str, object]:
    ...
    pages = [page_header(database.buffer_pool.peek_page(page_id), None if map_only else database, index_pages=index_pages)
             for page_id in range(offset, min(offset + limit, disk.page_count))]
    files = [{"name": database.path.name, "size_bytes": database.path.stat().st_size if database.path.exists() else 0}]
    return {"snapshot_at": datetime.now(timezone.utc).isoformat(), "readonly": True, "map_only": map_only,
             "files": files, "pages": pages, "total": disk.page_count, "offset": offset, "limit": limit,
             "page_size": disk.page_size, "next_page_id": metadata["next_page_id"],
            "free_pages": list(metadata["free_pages"])[offset:offset + limit],
             "free_page_count": len(metadata["free_pages"]), "named_pages": metadata["named_pages"],
             "system_tables": [table.to_dict() for table in database.catalog.system_tables()] if _is_admin(database) else "MASKED",
             "storage_revision": buffer_pool["revision"],
             "buffer_pool": buffer_pool, "io": disk.io_stats(),
...
def storage_page_changes(database: Database, since: int, limit: int) -> dict[str, object]:
    ...
    if truncated:
        return {"snapshot_at": datetime.now(timezone.utc).isoformat(), "readonly": True,
                "since": since, "revision": revision, "changed_page_ids": page_ids[:limit],
    ...
    pages = [page_header(database.buffer_pool.peek_page(page_id), database, index_pages=index_pages)
             for page_id in page_ids if 0 <= page_id < disk.page_count]
    return {"snapshot_at": datetime.now(timezone.utc).isoformat(), "readonly": True,
            "since": since, "revision": revision, "changed_page_ids": page_ids,
            "pages": pages, "total": disk.page_count, "page_size": disk.page_size,
            "free_pages": list(metadata["free_pages"]), "free_page_count": len(metadata["free_pages"]),
            "truncated": False}


def storage_cache_snapshot(database: Database, offset: int, limit: int) -> dict[str, object]:
...
def storage_index_snapshot(database: Database, limit: int) -> dict[str, object]:
...
def inspect_page(database: Database, page_id: int, offset: int, limit: int) -> dict[str, object]:
    authorize_storage(database)
    ...
    result.update({"readonly": True, "offset": offset, "limit": limit,
    ...
    if expose_raw:
        ...
        if page.page_type is PageType.HEAP:
            ...
            raw_text = json.dumps({"format": layout_preview.get("format"),
    ...
    if page.page_type == PageType.HEAP:
        ...
        for slot_id, raw in enumerate(slotted.slots):
            entry = layout_slots[slot_id] if slot_id < len(layout_slots) and isinstance(layout_slots[slot_id], dict) else {}
            ...
            if entry_deleted:
                ...
                page_offset = None
            byte_length = entry.get("length") if isinstance(entry.get("length"), int) else 0
            directory_offset = entry.get("directory_offset") if isinstance(entry.get("directory_offset"), int) else None
            if offset <= slot_id < offset + limit:
                slot_rows.append({"slot_id": slot_id, "deleted": raw is None,
                                  "record_bytes": len(raw) if raw else 0,
                                  "row": _display_row(raw, masked),
                                  "page_offset": page_offset, "byte_length": byte_length,
                                  "slot_directory_offset": directory_offset,
                                  "slot_directory_length": SLOT_ENTRY_SIZE,
                                  "storage_encoding": "JSON UTF-8 row bytes"})
        result["layout"] = "双向槽式页：槽目录向前增长，空闲片段按真实范围分布，JSON UTF-8 记录区从页尾向前增长。"
        result["slots"] = slot_rows
        result["total_slots"] = len(slotted.slots)
    elif page.page_type == PageType.CATALOG:
        result["catalog"] = {
            "tables": [table.to_dict() for table in database.catalog.tables()][offset:offset + limit],
            "system_tables": [table.to_dict() for table in database.catalog.system_tables()] if admin else "MASKED",
        ...
        result["index_node"] = index_page_info(page, offset=offset, limit=min(limit, 100))
        result["note"] = "真实落盘 B+Tree 节点；entries 是叶子 key/RowId，children 是内部页子节点。"
    else:
        result["note"] = "该页没有可展示的记录。"
    return result


def inspect_index(database: Database, name: str, offset: int, limit: int) -> dict[str, object]:
    authorize_storage(database)
    metadata = database.catalog.get_index(name)
    return {"metadata": metadata.to_dict(), **database.index_manager.get(name).snapshot(offset, limit)}
