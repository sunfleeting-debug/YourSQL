...
from ..common import ExecutionResult, YourSQLError, Value
...
from ..sql.plan import plan_from_statement
...
def _slice(sql: str, start: int, end: int) -> SQLSlice:
    ...
    return SQLSlice(sql[start:end], start, end, prefix.count("\n") + 1, len(prefix.rsplit("\n", 1)[-1]) + 1)
...
def redact_sql(sql: str) -> str:
    ...
    try:
        ...
        return " ".join("?" if token.kind in literals else token.lexeme
                        for token in tokenize(sql) if token.kind != TokenKind.EOF)
    except YourSQLError:
...
def _statement_anchor(source: SQLSlice) -> tuple[int, int]:
    ...
    try:
        token = next((item for item in tokenize(source.sql) if item.kind is not TokenKind.EOF), None)
    except YourSQLError:
        return source.line, source.column
    if token is None:
        return source.line, source.column
    return (token.line + source.line - 1,
...
def _keyword_suggestion(source: SQLSlice, error: YourSQLError) -> tuple[str, int, int, str] | None:
    ...
    expected_words = {item.upper() for item in expected.replace("'", "").split("/") if item.isalpha()}
    ...
    for index, token in enumerate(tokens):
        ...
        if token.lexeme == str(error.details.get("found", "")) and expected_rank != 0:
            continue
        position_distance = abs(token.line - error.line) * 1000 + abs(token.column - error.column)
        # 只看错误点之前的词，覆盖 “FROM t wher id = 1” 这种别名歧义场景。
        if token.line > error.line or (token.line == error.line and token.column > error.column):
            continue
        candidates.append((expected_rank, position_distance, suggestion, token.line, token.column, token.lexeme))
...
def error_info(error: Exception, source: SQLSlice, *, sensitive: bool = False) -> dict[str, object]:
    """返回带完整词素和轻量关键字建议的工作台诊断。"""
    if isinstance(error, YourSQLError):
        precise = error.line is not None
        if precise:
            line = (error.line or 1) + source.line - 1
            column = (error.column or 1) + (source.column - 1 if (error.line or 1) == 1 else 0)
        else:
            line, column = _statement_anchor(source)
        message = "敏感语句执行失败，详细值已隐藏" if sensitive else error.message
        diagnostic: dict[str, object] = {
            "code": error.code, "message": message, "line": line, "column": column,
            "position_accuracy": "token" if precise else "statement",
            "position_note": None if precise else "当前 Binder/执行器没有节点区间，定位到语句起点。",
        }
        if not sensitive:
            ...
            if suggestion is not None:
                ...
                diagnostic.update({
                    ...
                    "column": suggestion_column + (source.column - 1 if suggestion_line == 1 else 0),
        ...
        return diagnostic
    line, column = _statement_anchor(source)
    return {"code": "INTERNAL_ERROR", "message": "服务执行异常，请按请求 ID 检查服务日志",
            "line": line, "column": column, "position_accuracy": "statement"}


def stage(name: str, status: str, source: SQLSlice, *, data: object = None,
          duration_ms: float | None = None, reason: str | None = None,
          error: dict[str, object] | None = None) -> dict[str, object]:
    return {"name": name, "status": status, "data": data,
            "text": json.dumps(data, ensure_ascii=False, indent=2) if data is not None else reason or "",
            "duration_ms": duration_ms, "reason": reason, "error": error,
            "source": {**source.location(), "accuracy": "statement"}}


def compile_observed(database: Database, source: SQLSlice, stages: list[dict[str, object]]) -> CompilationResult:
    """逐阶段调用原编译器组件，实际执行复用同一 CompilationResult。"""
    sensitive = "IDENTIFIED" in source.sql.upper()

    def capture(name: str, operation: Callable[[], T], serialize: Callable[[T], object]) -> T:
        started = perf_counter()
        try:
            value = operation()
        except Exception as exc:
            stages.append(stage(name, "error", source, duration_ms=(perf_counter() - started) * 1000,
                                error=error_info(exc, source, sensitive=sensitive)))
            raise
        elapsed = (perf_counter() - started) * 1000
        stages.append(stage(name, "success", source,
                            data={"redacted": True, "reason": "凭据语句产物已隐藏"} if sensitive else serialize(value),
                            duration_ms=elapsed))
        return value

    tokens = capture("tokens", lambda: tuple(tokenize(source.sql)), lambda items: [item.as_dict() for item in items])
    statement = capture("ast", lambda: Parser(tokens).parse_one(), lambda node: node.to_dict())
    # WHY：绑定器会读取 Catalog；先检查对象权限，避免用编译接口探测无权表结构。
    database._authorize_statement(statement, database._action_for(statement))
    bound = capture("binding", lambda: Binder(database.catalog).bind(statement), lambda node: node.to_dict())
    plan = capture("logical_plan", lambda: plan_from_statement(statement), lambda node: node.to_dict())
    cache_sql = source.sql if isinstance(statement, (Select, Explain)) else None
    optimized_plan = capture("optimized_plan", lambda: database.optimize_plan(plan, sql=cache_sql),
                             lambda node: node.to_dict())
    # 当前执行器以优化后的 PlanNode 作为物理访问路径，尚未拆出独立物理算子树。
    physical_started = perf_counter()
    physical_data = {"redacted": True, "reason": "凭据语句产物已隐藏"} if sensitive else optimized_plan.to_dict()
    stages.append(stage("physical_plan", "partial", source, data=physical_data,
                        duration_ms=(perf_counter() - physical_started) * 1000,
                        reason="物理计划复用优化后的 PlanNode；独立物理规划器尚未拆分。"))
    return CompilationResult(tokens, statement, bound, plan, optimized_plan)


def result_columns(database: Database, statement: Statement, result: ExecutionResult) -> list[dict[str, object]]:
    ...
    if isinstance(statement, Select):
        ...
        for item in statement.items:
            ...
            if isinstance(expression, Star):
                for ref in references:
                    if not expression.table or expression.table.lower() in {ref.name.lower(), (ref.alias or "").lower()}:
                        declared.extend(column.data_type.value for column in database.catalog.get_relation(ref.name).schema)
            elif isinstance(expression, ColumnRef):
                matches = [column.data_type.value for ref in references
                           if not expression.table or expression.table.lower() in {ref.name.lower(), (ref.alias or "").lower()}
                           for column in database.catalog.get_relation(ref.name).schema if column.name.lower() == expression.name.lower()]
        ...
        declared = {"COLUMNS": ["VARCHAR", "VARCHAR", "VARCHAR", "VARCHAR", None],
                    "INDEX": ["VARCHAR", "VARCHAR", "BOOLEAN", "VARCHAR", "VARCHAR"]}.get(statement.target, ["VARCHAR"] * len(result.columns))
    columns = []
    for index, name in enumerate(result.columns):
        ...
        if not data_type:
            value = next((row[index] for row in result.rows if row[index] is not None), None)
            data_type = Value.infer(value).data_type.value if value is not None else "UNKNOWN"
