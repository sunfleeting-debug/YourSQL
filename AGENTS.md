...
- 使用中文本土化注释
- 使用必要的简短的头docsting
- 专门为 Web、Workbench、HTTP 前端接口或前端存储检查提供的模块、类、函数和展示 DTO，docstring 必须以醒目的 `【前端特供】` 开头，明确它们不是数据库核心逻辑。
- 混合模块只标记确实服务前端的符号，不要因为核心对象被前端调用、可以序列化 JSON 或有调试输出就整体标记；前端特供实现优先放在 `yoursql/engine/services/`。
- `【前端特供】` 是维护边界标记，不代表可以绕过权限、容量限制、脱敏或只读约束；新增前端适配仍必须复用核心接口并保持边界单向依赖。
- 对于内部注释，使用简要的HOW注释
