...
function updateJourney(phase, message){
 document.querySelectorAll('[data-step]').forEach(step=>{const active=step.dataset.step===phase;step.classList.toggle('current',active);if(active)step.setAttribute('aria-current','step');else step.removeAttribute('aria-current');});
 $('journey-caption').textContent=message;
 document.querySelector('.query-journey').classList.toggle('journey-error',phase==='error');
}
function welcome(){
 const box=empty('你的下一条发现，从这里开始','载入建表示例并执行，或从左侧选择已有数据表。');
 const actions=el('div',undefined,'empty-actions');
 const demo=el('button','＋ 载入建表示例','secondary');demo.addEventListener('click',()=>newQuery(example,'建表示例'));
 const write=el('button','编写 SQL →','text-button');write.addEventListener('click',()=>$('editor').focus());
 actions.append(demo,write);box.append(actions);return box;
}
...
function renderResults(){const content=$('result-content');content.replaceChildren();const result=state.results[state.resultIndex];$('export').disabled=state.view!=='data'||!result?.ok||!result?.columns.length;$('result-filter').disabled=state.view!=='data'||!result?.ok||!result?.columns.length;$('statement-label').hidden=!['data','tokens','ast','semantic','plan','optimizer','execute'].includes(state.view)||state.results.length<2;
 ...
 if(!result){content.append(welcome());return;}
