...
新建数据库没有任何表，这是正常情况。点击右上角“插入建表示例”，再点击“执行 SQL”，即可创建 `students` 表并查询示例记录。已有同名表时不要重复执行建表示例。
...
查询标签和历史仅保留在当前页面，刷新或关闭前请保存需要的 SQL。表格展示及 CSV 导出最多包含前 1,000 行，完整查询仍由引擎执行。
