...
export default function HistoryPanel({ history, refresh, inspect }: { history: History | null; refresh: () => void; inspect: (id: string) => void }) {
  ...
  return (
    <section className="history-region" aria-label="查询历史">
      <div className="panel-heading">
        <strong>
          <Clock3 size={14} />
          查询历史
        </strong>
        <button className="icon-button" aria-label="刷新查询历史" onClick={refresh}>
          <RefreshCw size={13} />
        </button>
      </div>
      <input
      ...
      />
      {visible.length === 0 ? (
        <p className="side-empty">暂无查询记录</p>
      ) : (
        visible.map(item => (
          <button
            key={item.id}
          ...
          >
            ...
            <code>{item.sql}</code>
            <small>{elapsed(item.elapsed_ms)}</small>
      ...
      {items.length > 6 && (
        <div className="small-pagination">
          <button aria-label="上一页历史" disabled={!page} onClick={() => setPage(page - 1)}>
            <ChevronLeft size={13} />
          </button>
          <span>
            {page + 1} / {Math.ceil(items.length / 6)}
          </span>
          <button aria-label="下一页历史" disabled={(page + 1) * 6 >= items.length} onClick={() => setPage(page + 1)}>
            <ChevronRight size={13} />
          </button>
        </div>
      )}
      <p className="history-note" title={history?.retention}>
        最近 200 条 · 字面量已脱敏 · 不可直接重放
      </p>
    </section>
  )
}
