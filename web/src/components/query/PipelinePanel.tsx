  ...
  return (
    <div className="token-table-wrap">
      <table className="token-table">
        ...
        <tbody>
          {rows.slice(0, 500).map((token, index) => (
            <tr key={`${String(token.lexeme)}-${index}`}>
              <td>{index + 1}</td>
              <td>
                <code>{display(token.kind)}</code>
              </td>
              <td className="token-lexeme">
                <code>{display(token.lexeme)}</code>
              </td>
              <td>{token.value === undefined ? <span className="muted">—</span> : <code>{display(token.value)}</code>}</td>
              <td>
                <code>
                  Ln {display(token.line)}, Col {display(token.column)}
                </code>
              </td>
            </tr>
          ))}
      ...
      {rows.length > 500 && <p className="hint">仅显示前 500 项</p>}
    </div>
  )
}
  ...
  return (
    <div className="semantic-summary">
      <div className="semantic-result">
        <span className="semantic-check">✓</span>
        <div>
          <strong>检查通过</strong>
          <small>对象、列和输入信息可以进入计划生成</small>
        </div>
      </div>
      <dl className="semantic-facts">
        <div>
          <dt>语句</dt>
          <dd>{summary.statement}</dd>
        </div>
        <div>
          <dt>对象</dt>
          <dd>{summary.tables}</dd>
        </div>
        <div>
          <dt>结果列</dt>
        ...
        <div>
          <dt>绑定</dt>
          <dd>{summary.binding}</dd>
        </div>
      </dl>
      <details className="semantic-raw">
        <summary>绑定详情</summary>
        <JsonTree value={value} defaultExpandedDepth={0} />
      </details>
    </div>
  )
}
...
function StageError({ error }: { error: DBError }) {
  ...
  return (
    <div className="stage-error" role="alert">
      ...
      <p>{error.message}</p>
      {error.position_note && <small>{error.position_note}</small>}
    </div>
  )
}
...
function PlanDelta({ before, after }: { before: StageValue; after: StageValue }) {
  ...
  const rows = Array.from(new Set([...beforeScans.map(item => item.table), ...afterScans.map(item => item.table)]))
    ...
    .map(table => ({
      table,
      before: beforeScans.find(item => item.table === table)?.kind ?? '—',
      after: afterScans.find(item => item.table === table)?.kind ?? '—'
    }))
  return (
    <div className="plan-delta">
      ...
      {rows.map(row => (
        <div className="plan-delta-row" key={row.table}>
          <code>{row.table}</code>
          <span>{row.before}</span>
          <b>→</b>
          <span className={row.before === row.after ? 'unchanged' : 'changed'}>{row.after}</span>
        </div>
      ))}
    </div>
  )
...
function PlanState({
  ...
  return (
    <article className={`plan-state plan-state-${tone}`}>
      <header className="plan-state-heading">
        <div className="plan-state-title">
          <span className="plan-state-index">{tone === 'before' ? '01' : '02'}</span>
          <div>
            <strong>{label}</strong>
            <small>{subtitle}</small>
          </div>
        </div>
        <div className="plan-state-meta">
          <span className={`stage-detail-status ${stage?.status ?? 'unsupported'}`}>{statusLabel(stage)}</span>
          {stage?.duration_ms != null && <span>{elapsed(stage.duration_ms)}</span>}
        </div>
      </header>
...
function PlanComparison({ before, after }: { before: Stage | undefined; after: Stage | undefined }) {
  return (
    <div className="plan-comparison">
      <div className="plan-comparison-intro">
        <div>
          <strong>优化前后对比</strong>
          <span>纵向对齐算子结构与访问路径</span>
        </div>
        <div className="plan-comparison-legend">
          <span className="before">优化前</span>
          <b>→</b>
          <span className="after">优化后</span>
        </div>
      </div>
      <PlanState stage={before} label="优化前" subtitle="逻辑计划" value={before?.data} tone="before" />
      <div className="plan-transition" aria-label="优化器变更摘要">
        <div className="plan-transition-rule">
          <span>优化器</span>
        </div>
        {before?.data !== undefined && after?.data !== undefined && <PlanDelta before={before.data} after={after.data} />}
      </div>
...
export default function PipelinePanel({ stages }: { stages: Stage[] }) {
  ...
  const groups = useMemo(() => {
    ...
    return groupDefinitions.map(group => ({ ...group, keys: group.keys.filter(key => stageMap.has(key)) })).filter(group => group.keys.length)
  }, [stageMap, stages])
    ...
    <div className="pipeline">
      <div className="pipeline-overview pipeline-stage-picker" aria-label="查询流水线阶段">
        <label className="pipeline-select-wrap">
          <select value={selected} onChange={event => setSelected(event.target.value)} aria-label="选择流水线阶段">
            {groups.map(group => (
              <optgroup label={group.label} key={group.label}>
                {group.keys.map(key => (
                  <option value={key} key={key}>
                    {optionLabel(key, stageMap.get(key))}
                  </option>
                ))}
              </optgroup>
            ))}
      ...
      <section className="stage-detail">
        <div className="detail-heading stage-detail-heading">
          ...
          {selected !== 'logical_plan' && (
            <div className="stage-meta">
              <span className={`stage-detail-status ${current?.status ?? 'unsupported'}`}>{statusLabel(current)}</span>
              {current?.duration_ms != null && <span>{elapsed(current.duration_ms)}</span>}
            </div>
          )}
        </div>
        {selected === 'logical_plan' ? (
          <PlanComparison before={current} after={optimized} />
        ) : !current ? (
