...
function buildItem(value: unknown, key = 'root'): AstItem | null {
  ...
  if (Array.isArray(value)) {
    ...
    return {
      id,
      label: childLabel(key),
      value: `${value.length} 项`,
      children: value.map((item, index) => buildItem(item, `${key}[${index + 1}]`)).filter((item): item is AstItem => item !== null)
    }
  }
...
function Branch({ item, depth }: { item: AstItem; depth: number }) {
  ...
  return (
    <div className={`ast-tree-branch depth-${Math.min(depth, 3)}`}>
      <button
        className={`ast-tree-node ${hasChildren ? '' : 'leaf'}`}
        aria-expanded={hasChildren ? expanded : undefined}
        onClick={() => hasChildren && setExpanded(value => !value)}
      >
        {hasChildren && <span className="ast-tree-chevron">{expanded ? '▾' : '▸'}</span>}
        <strong>{item.label}</strong>
        {item.value !== undefined && <code>{item.value}</code>}
      </button>
...
export default function AstTree({ value }: { value: unknown }) {
  ...
  return root ? (
    <div className="ast-tree" aria-label="AST 结构树">
      <Branch item={root} depth={0} />
    </div>
  ) : (
    <div className="stage-empty">无法识别 AST</div>
  )
}
