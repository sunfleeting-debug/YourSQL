...
interface Props {
  value: string; onChange: (value: string) => void; tables: TableMeta[]; dialect: Dialect | null
  execute: (all: boolean) => void; onCursor: (position: number) => void; onSelection: (from: number, to: number) => void
  error: DBError | null; running: boolean; editorRef: React.RefObject<EditorView | null>
}
...
class ErrorLensWidget extends WidgetType {
  ...
  eq(other: ErrorLensWidget): boolean {return this.message === other.message}
...
const errorLens = StateField.define({
  ...
  update(decorations, change) {
    ...
    const widgets = effect.value
      ...
      .map(diagnostic => Decoration.widget({widget: new ErrorLensWidget(diagnostic.message), side: 1}).range(
        change.state.doc.lineAt(diagnostic.from).to))
...
const activeStatement = StateField.define({
  ...
  update(_value, change) {
    ...
    return Decoration.set(Array.from({length: to - from + 1}, (_, i) =>
      Decoration.line({class: 'cm-activeStatement'}).range(change.state.doc.line(from + i).from)))
  },
  provide: field => EditorView.decorations.from(field),
})
...
export default function SqlEditor(props: Props) {
  ...
  useEffect(() => {
    ...
    const complete = (context: CompletionContext) => {
      ...
      for (const table of current.tables) {
        ...
        for (const column of table.columns) unique.set(column.name, {label: column.name, type: 'property', detail: `${table.name} · ${column.type}`, apply: quoteName(column.name)})
      }
    ...
    const view = new EditorView({
      state: EditorState.create({doc: live.current.value, extensions: [
        basicSetup, syntaxHighlighting(sqlHighlightStyle), activeStatement, errorLens,
        language.current.of(sql()), editable.current.of(EditorView.editable.of(true)),
        EditorView.contentAttributes.of({'aria-label': 'SQL 编辑器', 'spellcheck': 'false'}),
        keymap.of([
          {key: 'Tab', run: handleTab},
          {key: 'F5', run: () => {live.current.execute(false); return true}},
          {key: 'Shift-F5', run: () => {live.current.execute(true); return true}},
        ]),
        ...
        EditorView.theme({
          '&': {height: '100%', fontSize: '14px'}, '.cm-scroller': {fontFamily: "'Cascadia Code', Consolas, monospace", lineHeight: '1.8'},
          '.cm-content': {padding: '16px 0'}, '.cm-gutters': {background: '#fff', border: 'none', color: '#94a1b3', paddingRight: '12px'},
          '.cm-line': {paddingLeft: '12px'}, '.cm-activeLineGutter': {background: '#e7f4f2', color: '#087f78'},
          '.cm-activeLine': {background: 'transparent'}, '.cm-cursor': {borderLeftColor: '#087f78'},
          '&.cm-focused': {outline: 'none'},
          // HOW：用左侧内阴影提示当前语句，避免整行背景盖住文本选区。
          '.cm-activeStatement': {background: 'transparent', boxShadow: 'inset 2px 0 #b9ddd8'},
          '.cm-errorLens': {
            display: 'inline-block', color: '#b03b42', background: '#fff5f5', border: '1px solid #f0d4d7',
            borderRadius: '3px', padding: '1px 6px', marginLeft: '10px', fontSize: '11px', lineHeight: '1.5',
            fontFamily: "Inter, 'Microsoft YaHei', sans-serif", whiteSpace: 'normal', overflowWrap: 'anywhere', verticalAlign: 'middle',
          },
    ...
    return () => {view.destroy(); live.current.editorRef.current = null}
  }, [])
  ...
  useEffect(() => {
    ...
    if (dialect) props.editorRef.current?.dispatch({effects: language.current.reconfigure(sql({dialect: SQLDialect.define({
      ...
      doubleQuotedStrings: false, backslashEscapes: true, identifierQuotes: '`"',
    })}))})
  ...
  useEffect(() => {
    const view = props.editorRef.current, error = props.error
    if (!view || !error) return
    const text = view.state.doc.toString()
    const range = diagnosticRange(text, error.line, error.column)
    if (!range) return
    view.dispatch(setDiagnostics(view.state, [{...range, severity: 'error', message: error.message}]))
    view.dispatch({selection: {anchor: range.from}, effects: EditorView.scrollIntoView(range.from, {y: 'center'}) as StateEffect<unknown>})
    view.focus()
