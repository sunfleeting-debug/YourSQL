...
const workspaceCSS = readFileSync(fileURLToPath(new URL('./styles/workbench.css', import.meta.url)), 'utf8')

describe('存储页常驻右栏', () => {
  it('两栏列宽只由 has-rail 决定，不随块选中开关抽屉改变地图列数', () => {
    expect(workspaceCSS).toContain('.storage-mode-layout.has-rail')
    // HOW：桌面端让页面地图优先获得空间，同时给详情内容保留明确的最低宽度。
    expect(workspaceCSS).toContain('minmax(330px, 36%)')
