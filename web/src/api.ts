...
export class ApiError extends Error {
  constructor(public status: number, public detail: DBError, public requestId = '') {
...
export async function api<T>(path: string, body?: object, signal?: AbortSignal): Promise<T> {
  ...
  try {
    ...
    throw new ApiError(0, {code: 'NETWORK_ERROR', message: '连接中断或请求超时。已提交的任务可能仍在运行，请恢复连接后查看。'})
  }
...
export async function upload<T>(path: string, file: File, signal?: AbortSignal): Promise<T> {
  ...
  try {
    response = await fetch(path, {
      ...
      headers: {'Content-Type': 'application/octet-stream', 'X-YourSQL-Client': 'workbench',
        'X-YourSQL-File-Name': encodeURIComponent(file.name)},
      body: file, signal: signal ?? AbortSignal.timeout(60_000),
    })
  } catch (error) {
    if (signal?.aborted) throw error
